...
export default function PatientForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const inputClass = "p-3 border border-gray-300 rounded-lg text-gray-900 placeholder:text-gray-500 bg-white focus:outline-none focus:ring-2 focus:ring-teal-500";
  ...
  return (
    <form
      ...
      className="bg-white rounded-lg shadow-md p-6 space-y-4"
    >
      <h3 className="text-lg font-bold text-gray-900">Nouveau patient</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <input
          name="nom"
          placeholder="Nom"
      ...
      <button
        type="submit"
        disabled={loading}
        className="bg-teal-600 text-white px-6 py-3 rounded-lg hover:bg-teal-700 transition disabled:opacity-50 font-semibold"
      >
        {loading ? "Enregistrement..." : "Enregistrer"}
      </button>
    </form>
